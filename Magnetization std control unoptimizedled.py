...
# May 2018 - Feb 2023
# Example program by Jordi Faraudo edited by Luis Martinez
...
minstd=float(input("\n Enter the standard deviation goal(suggestion 0.0015):"))
#Init Magnetization and Energy
step=[]
M=[]
E=[]
#Make a list of the Energies and Magnetization reduced to the last 100 values
Ered= []
Mred= []
...
plt.ion()
#Make empty list of the standard deviation for Energy and Magnetization
Estd = []
Mstd = []    
...
for i in range(msrmnt):
            ...
            Ered.append(Ene)
            Mred.append(Mag)
            #Erease the first value of the list to calculate the standard deviation 
            if len(Ered)>100:
                    del Ered[0]
            if len(Mred)>100:
                    del Mred[0]
            #Calculate the standard deviation
            Estdcalc = np.std(Ered)
            Mstdcalc= np.std(Mred)
            Estd.append(Estdcalc)
            Mstd.append(Mstdcalc)
            if Mstd[i]<minstd:
                    break
                
            

            #plot certain configurations and its standard deviations
            if t%10 == 0:
                print('\nMC step=',t,' Energy=',Ene,' M=',Mag, 'Energy std=', Estdcalc, 'Magnetization std=',Mstdcalc)
                print(config)
